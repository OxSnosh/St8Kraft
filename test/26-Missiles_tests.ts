...
describe("Missiles Contract", function () {
    ...
    describe("Buying Cruise Missiles", function () {
        ...
        it("tests that buyCruiseMissiles works correctly", async function () {
            ...
            await expect(missilescontract.connect(signer1).buyCruiseMissiles(5, 1)).to.be.revertedWith("!nation owner")
            await expect(missilescontract.connect(signer1).buyCruiseMissiles(5, 0)).to.be.revertedWith("requires 15 tech")
            await technologymarketcontrat.connect(signer1).buyTech(0, 10)
