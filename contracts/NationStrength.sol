...
contract NationStrengthContract is Ownable {
    ...
    constructor (address _infrastructure, address _forces, address _fighters, address _bombers, address _navy) {
    ...
    function getNationStrength(uint256 id) public view returns(uint256) {
        uint256 nationStrengthFromCommodities = getNationStrengthFromCommodities(id);
        uint256 nationStrengthFromMilitary = getNationStrengthFromMilitary(id);
        uint256 nationStrength = nationStrengthFromCommodities + nationStrengthFromMilitary;
        return nationStrength;
    }

    function getNationStrengthFromCommodities(uint256 id) internal view returns(uint256) {
        ...
        uint256 strengthFromCommodities = (((landCount * 150) + (techCount * 500) + (infrastructureCount * 300))/100);
    ...
    function getNationStrengthFromMilitary(uint256 id) internal view returns(uint256) {
        ...
        uint256 tankStrength = (((deployedTankCount * 15)+(defendingTankCount * 20))/100);
    ...
    function getStrengthFromFighters(uint256 id) internal view returns (uint256) {
        ...
        uint256 additionalFighterStrength = getAdditionalStrengthFromFighters(id);
        uint256 strengthFromFighters = ((
            (yak9Count * 1) +
            (p51MustangCount * 2) +
            (f86SabreCount * 3) +
            (mig15Count * 4) +
            (f100SuperSabreCount * 5) +
            additionalFighterStrength
        ) * 5);
        return strengthFromFighters;
    }

    function getAdditionalStrengthFromFighters(uint256 id) public view returns (uint256) {
        ...
        uint256 additionalFighterStrength = (
            (f35LightningCount * 6) +
            (f15EagleCount * 7) +
            (su30MkiCount * 8) +
            (f22RaptorCount * 9)
    ...
    function getStrengthFromBombers(uint256 id) internal view returns (uint256) {
        ...
        uint256 additionalStrengthFromBombers = getAdditionalStrengthFromBombers(id);
        uint256 strengthFromBombers = ((
            (ah1CobraCount * 1) +
            (ah64ApacheCount * 2) +
            (bristolBlenheimCount * 3) +
            (b52MitchellCount * 4) +
            (b17gFlyingFortressCount * 5) +
            additionalStrengthFromBombers
        ) * 5);
        return strengthFromBombers;
    }

    function getAdditionalStrengthFromBombers (uint256 id) internal view returns (uint256) {
        ...
        uint256 additionalStrengthFromBombers = (
            (b52StratofortressCount * 6) +
            (b2SpiritCount * 7) +
            (b1bLancerCount * 8) +
            (tupolevTu160Count * 9)
        );
    ...
    function getStrengthFromNavy(uint256 id) internal view returns (uint256) {
        ...
        uint256 strengthFromNavy = ((
            (corvetteCount * 1) +
            (landingShipCount * 3) +
            (battleshipCount * 5) +
            (cruiserCount * 6) +
            additionalNavyStrength
        ) * 10);
    ...
    function getAdditionalNavyStrength(uint256 id) internal view returns (uint256) {
        ...
        uint256 additionalNavyStrength = ( 
            (frigateCount * 8) +
            (destroyerCount * 11) +
            (submarineCount * 12) +
