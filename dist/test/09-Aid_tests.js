...
describe("Aid Contract", function () {
    ...
    describe("Aid Contract", function () {
        it("aid1 tests proposeAid() function", async function () {
            ...
            (0, chai_1.expect)(proposalDetails[0].toNumber()).to.equal(0);
            (0, chai_1.expect)(proposalDetails[2].toNumber()).to.equal(0);
            (0, chai_1.expect)(proposalDetails[3].toNumber()).to.equal(1);
            (0, chai_1.expect)(proposalDetails[4].toNumber()).to.equal(100);
            (0, chai_1.expect)(proposalDetails[5]).to.equal(BigInt(5999999999999999899336704));
            (0, chai_1.expect)(proposalDetails[6].toNumber()).to.equal(4000);
        });
        it("aid1 tests acceptProposal() function", async function () {
            await aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4000);
            await aidcontract.connect(signer2).acceptProposal(0);
            await (0, chai_1.expect)((await forcescontract.getSoldierCount(1)).toNumber()).to.equal(4020);
            await (0, chai_1.expect)((await infrastructurecontract.getTechnologyCount(1)).toNumber()).to.equal(100);
            await (0, chai_1.expect)((await (await treasurycontract.checkBalance(1)))).to.equal(BigInt("1008000000000000013186891776"));
        });
        it("aid1 tests proposeAid() function reverts correctly with slot not available", async function () {
            await (0, chai_1.expect)(aidcontract.connect(signer1).proposeAid(1, 1, 100, BigInt(6000000 * (10 ** 18)), 4000)).to.be.revertedWith("!nation ruler");
            await aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4000);
            await aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4000);
            await aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4000);
            await aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4000);
            ...
            await aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4000);
            await aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4000);
            await (0, chai_1.expect)(aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4000)).to.be.revertedWith("aid slot not available");
        });
        it("aid1 tests proposeAid() function reverts correctly when aid not available", async function () {
            ...
            await (0, chai_1.expect)(aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4000)).to.be.revertedWith("not enough tech for this proposal");
            await technologymarketcontrat.connect(signer1).buyTech(0, 10000);
            await treasurycontract.connect(signer1).withdrawFunds(BigInt(992600000000000004764669344), 0),
                await (0, chai_1.expect)(aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4000)).to.be.revertedWith("not enough funds for this porposal");
            ...
            await (0, chai_1.expect)(aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4000)).to.be.revertedWith("not enough soldiers for this porposal");
            await forcescontract.connect(signer1).buySoldiers(5000, 0);
        });
        it("aid1 tests proposeAid() function reverts when max amount exceeded (no frderal aid)", async function () {
            await (0, chai_1.expect)(aidcontract.connect(signer1).proposeAid(0, 1, 101, BigInt(6000000 * (10 ** 18)), 4000)).to.be.revertedWith("max tech exceeded");
            await (0, chai_1.expect)(aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000001 * (10 ** 18)), 4000)).to.be.revertedWith("max balance excedded");
            await (0, chai_1.expect)(aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4001)).to.be.revertedWith("max soldier aid is excedded");
        });
        it("aid1 tests proposeAid() function doesnt revert with federal aid", async function () {
            ...
            (0, chai_1.expect)(proposalDetails[2].toNumber()).to.equal(0);
            (0, chai_1.expect)(proposalDetails[3].toNumber()).to.equal(1);
            (0, chai_1.expect)(proposalDetails[4].toNumber()).to.equal(101);
            (0, chai_1.expect)(proposalDetails[5]).to.equal(BigInt(6000001000000000312213504));
            (0, chai_1.expect)(proposalDetails[6].toNumber()).to.equal(4001);
        });
        it("aid1 tests acceptProposal() function reverts when proposal expires", async function () {
            ...
            await keepercontract.incrementGameDay();
            await keepercontract.incrementGameDay();
            await (0, chai_1.expect)(aidcontract.connect(signer2).acceptProposal(0)).to.be.revertedWith("proposal expired");
        });
        it("aid1 tests acceptProposal() function reverts when proposal is cancelled by sender", async function () {
            ...
            (0, chai_1.expect)(array[1]).to.equal(true);
            await (0, chai_1.expect)(aidcontract.connect(signer2).acceptProposal(0)).to.be.revertedWith("this offer has been cancelled");
        });
        ...
        it("aid1 tests acceptProposal() function reverts when proposal aleady accepted", async function () {
            await aidcontract.connect(signer1).proposeAid(0, 1, 100, BigInt(6000000 * (10 ** 18)), 4000);
            await aidcontract.connect(signer2).acceptProposal(0);
            await (0, chai_1.expect)(aidcontract.connect(signer2).acceptProposal(0)).to.be.revertedWith("this offer has been accepted already");
        });
        it("aid1 tests acceptProposal() function reverts when proposal is deficient", async function () {
            ...
            await treasurycontract.connect(signer1).withdrawFunds(BigInt(992600000000000004764669344), 0),
                await (0, chai_1.expect)(aidcontract.connect(signer2).acceptProposal(0)).to.be.revertedWith("not enough funds for this porposal");
        });
        ...
        it("aid1 tests that max aid proposals increases from 10 to 13 with a disaster relief agency", async function () {
            ...
            (0, chai_1.expect)(maxAidSlots.toNumber()).to.equal(10);
            ...
            (0, chai_1.expect)(maxAidSlots2.toNumber()).to.equal(13);
        });
        it("aid1 tests that aid proposals last 10 days decreases when an aid proposal is cancelled", async function () {
            ...
            (0, chai_1.expect)(proposals.toNumber()).to.equal(1);
            await aidcontract.connect(signer1).cancelAid(0);
            let proposals2 = await aidcontract.getAidProposalsLast10Days(0);
            // console.log(proposals2.toNumber())
            (0, chai_1.expect)(proposals2.toNumber()).to.equal(0);
        });
    });
});
