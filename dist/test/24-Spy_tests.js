...
describe("Spy Contract", function () {
    let warbucks;
    ...
    describe("Buy Spies", function () {
        it("tests that default max spy count works", async function () {
            ...
            (0, chai_1.expect)(maxSpies).to.equal(50);
        });
        it("tests that max spy count changes with intel agencies", async function () {
            ...
            var maxSpies = await spycontract.getMaxSpyCount(0);
            // console.log((maxSpies).toNumber())
            (0, chai_1.expect)(maxSpies).to.equal(550);
        });
        it("tests that max spy count changes with intel agencies and CIA", async function () {
            ...
            var maxSpies = await spycontract.getMaxSpyCount(0);
            // console.log((maxSpies).toNumber())
            (0, chai_1.expect)(maxSpies).to.equal(800);
        });
        it("tests that updateSpyPrice works", async function () {
            var cost = await spycontract.getSpyPrice();
            (0, chai_1.expect)(cost).to.equal(100000);
            await spycontract.connect(signer0).updateSpyPrice(200000);
            var cost2 = await spycontract.getSpyPrice();
            (0, chai_1.expect)(cost2).to.equal(200000);
        });
        it("tests that buySpies works correctly", async function () {
            await (0, chai_1.expect)(spycontract.connect(signer1).buySpies(60, 0)).to.be.revertedWith("cannot own that many spies");
            await (0, chai_1.expect)(spycontract.connect(signer1).buySpies(40, 1)).to.be.revertedWith("!nation owner");
            await spycontract.connect(signer1).buySpies(40, 0);
            var spyCount = await spycontract.getSpyCount(0);
            (0, chai_1.expect)(spyCount).to.equal(40);
        });
        it("tests that spies can be decommissioned", async function () {
            ...
            (0, chai_1.expect)(spyCount2).to.equal(20);
        });
    });
});
