...
async function testFrontEndError() {
    ...
    await warbucks.transferFrom(signers[0].address, signers[1].address, BigInt(4000000 * 10**18))

    console.log((await warbucks.balanceOf(signers[1].address)).toString())

    let CountryMinterAbi = metadata.HARDHAT.countryminter.ABI
    let CountryMinterAddress = metadata.HARDHAT.countryminter.address

    const countryMinter = new ethers.Contract(CountryMinterAddress, CountryMinterAbi, provider) as CountryMinter

    await countryMinter.connect(signers[1].address).generateCountry("Ruler", "Flag", "Description", "Text",)
}
