...
describe("Missiles Contract", function () {
    let warbucks;
    ...
    describe("Buying Cruise Missiles", function () {
        it("tests that updateCruiseMissileCost works correctly", async function () {
            var cost = await missilescontract.getCruiseMissileCost(0);
            (0, chai_1.expect)(cost).to.equal(BigInt(20000 * (10 ** 18)));
            await missilescontract.connect(signer0).updateCruiseMissileCost(BigInt(30000 * 10 ** 18));
            var cost = await missilescontract.getCruiseMissileCost(0);
            (0, chai_1.expect)(cost).to.equal(BigInt(30000 * (10 ** 18)));
        });
        it("tests that getCruiseMissileCost works correctly", async function () {
            var cost = await missilescontract.getCruiseMissileCost(0);
            (0, chai_1.expect)(cost).to.equal(BigInt(20000 * (10 ** 18)));
            await resourcescontract.connect(signer0).mockResourcesForTesting(0, 8, 1);
            var cost = await missilescontract.getCruiseMissileCost(0);
            (0, chai_1.expect)(cost).to.equal(BigInt(16000 * (10 ** 18)));
            ...
            var cost = await missilescontract.getCruiseMissileCost(0);
            (0, chai_1.expect)(cost).to.equal(BigInt(11000 * (10 ** 18)));
            // console.log(cost/(10**18))
        });
        it("tests that buyCruiseMissiles works correctly", async function () {
            var count = await missilescontract.getCruiseMissileCount(0);
            (0, chai_1.expect)(count).to.equal(0);
            await (0, chai_1.expect)(missilescontract.connect(signer1).buyCruiseMissiles(5, 1)).to.be.revertedWith("!nation owner");
            await (0, chai_1.expect)(missilescontract.connect(signer1).buyCruiseMissiles(5, 0)).to.be.revertedWith("requires 15 tech");
            await technologymarketcontrat.connect(signer1).buyTech(0, 10);
            await missilescontract.connect(signer1).buyCruiseMissiles(5, 0);
            var count = await missilescontract.getCruiseMissileCount(0);
            (0, chai_1.expect)(count).to.equal(5);
        });
    });
    describe("Buying Nukes", function () {
        it("tests that updateDefaultNukeCost works correctly", async function () {
            var defualtCost = await missilescontract.getDefaultNukeCost();
            var cost = BigInt(defualtCost / (10 ** 18));
            // console.log(cost)
            (0, chai_1.expect)(cost).to.equal(BigInt(500000));
            await missilescontract.connect(signer0).updateDefaultNukeCost(BigInt(400000 * (10 ** 18)));
            var defualtCost = await missilescontract.getDefaultNukeCost();
            // console.log(BigInt(defualtCost))
            (0, chai_1.expect)(defualtCost).to.equal(BigInt(399999999999999966445568));
        });
        it("tests that nuke cost updates correctly", async function () {
            var defualtCost = await missilescontract.getNukeCost(0);
            var cost = BigInt(defualtCost / (10 ** 18));
            // console.log(cost)
            (0, chai_1.expect)(cost).to.equal(BigInt(500000));
            await resourcescontract.connect(signer0).mockResourcesForTesting(0, 8, 1);
            ...
            // console.log(cost)
            (0, chai_1.expect)(defualtCost).to.equal(BigInt("400000000000000000000000"));
        });
        it("tests that buyNukes reverts correctly", async function () {
            await (0, chai_1.expect)(missilescontract.connect(signer1).buyNukes(1)).to.be.revertedWith("!nation owner");
            await (0, chai_1.expect)(missilescontract.connect(signer1).buyNukes(0)).to.be.revertedWith("requires 75 tech");
            await technologymarketcontrat.connect(signer1).buyTech(0, 75);
            await (0, chai_1.expect)(missilescontract.connect(signer1).buyNukes(0)).to.be.revertedWith("insufficient infrastructure");
            await infrastructuremarketplace.connect(signer1).buyInfrastructure(0, 600);
            await (0, chai_1.expect)(missilescontract.connect(signer1).buyNukes(0)).to.be.revertedWith("no uranium");
            await resourcescontract.connect(signer0).mockResourcesForTesting(0, 17, 1);
            await (0, chai_1.expect)(missilescontract.connect(signer1).buyNukes(0)).to.be.revertedWith("nation strength too low");
            ...
            await (0, chai_1.expect)(missilescontract.connect(signer1).buyNukes(0)).to.be.revertedWith("already purchased nuke today");
        ...
        it("tests that getNukesPurchasedToday() works correctly", async function () {
            var count = await missilescontract.getNukesPurchasedToday(0);
            (0, chai_1.expect)(count).to.equal(0);
            await resourcescontract.connect(signer0).mockResourcesForTesting(0, 17, 1);
            await infrastructuremarketplace.connect(signer1).buyInfrastructure(0, 2000);
            ...
            await wonderscontract2.connect(signer1).buyWonder2(0, 8);
            await missilescontract.connect(signer1).buyNukes(0);
            var count2 = await missilescontract.getNukesPurchasedToday(0);
            (0, chai_1.expect)(count2).to.equal(1);
            await keepercontract.connect(signer0).incrementGameDay();
            var count3 = await missilescontract.getNukesPurchasedToday(0);
            (0, chai_1.expect)(count3).to.equal(0);
        });
    });
    describe("Affect of Nuke Attacks", function () {
        it("tests that a nuke attack will decrease cruise missile count", async function () {
            await technologymarketcontrat.connect(signer2).buyTech(1, 75);
            await missilescontract.connect(signer2).buyCruiseMissiles(25, 1);
            var count = await missilescontract.getCruiseMissileCount(1);
            (0, chai_1.expect)(count).to.equal(25);
            ...
            var count2 = await missilescontract.getCruiseMissileCount(1);
            // console.log(count2.toNumber(), "cruise missiles after nuke attack")
            (0, chai_1.expect)(count2).to.equal(17);
        });
        it("tests that a nuke attack will decrease cruise missile count less if defender has a fallout shelter", async function () {
            await technologymarketcontrat.connect(signer2).buyTech(1, 75);
            await missilescontract.connect(signer2).buyCruiseMissiles(25, 1);
            var count = await missilescontract.getCruiseMissileCount(1);
            (0, chai_1.expect)(count).to.equal(25);
            ...
            var count2 = await missilescontract.getCruiseMissileCount(1);
            // console.log(count2.toNumber(), "cruise missiles after nuke attack")
            (0, chai_1.expect)(count2).to.equal(19);
        });
    });
});
