...
describe("Ground Battle Contract", function () {
    let warbucks;
    ...
    describe("Ground Battle Works", function () {
        it("tests that ground battle gets set up correctly", async function () {
            ...
            var attackerStruct = await groundbattlecontract.returnAttackerForcesStruct(0);
            // console.log(attackerStruct[0].toNumber(), "attack type")
            (0, chai_1.expect)(attackerStruct[0].toNumber()).to.equal(1);
            // console.log(attackerStruct[1].toNumber(), "soldiers deployed")
            (0, chai_1.expect)(attackerStruct[1].toNumber()).to.equal(1000);
            // console.log(attackerStruct[2].toNumber(), "tanks deployed")
            (0, chai_1.expect)(attackerStruct[2].toNumber()).to.equal(30);
            // console.log(attackerStruct[3].toNumber(), "attacker strength")
            (0, chai_1.expect)(attackerStruct[3].toNumber()).to.equal(1450);
            // console.log(attackerStruct[4].toNumber(), "attacker Id")
            (0, chai_1.expect)(attackerStruct[4].toNumber()).to.equal(0);
            // console.log(attackerStruct[5].toNumber(), "war ID")
            (0, chai_1.expect)(attackerStruct[5].toNumber()).to.equal(0);
            var defenderStruct = await groundbattlecontract.returnDefenderForcesStruct(0);
            // console.log(defenderStruct[0].toNumber(), "soldiers defending")
            (0, chai_1.expect)(defenderStruct[0].toNumber()).to.equal(1020);
            // console.log(defenderStruct[1].toNumber(), "tanks defending")
            (0, chai_1.expect)(defenderStruct[1].toNumber()).to.equal(20);
            // console.log(defenderStruct[2].toNumber(), "defender strength")
            (0, chai_1.expect)(defenderStruct[2].toNumber()).to.equal(2040);
            // console.log(defenderStruct[3].toNumber(), "defender Id")
            (0, chai_1.expect)(defenderStruct[3].toNumber()).to.equal(1);
            // console.log(defenderStruct[4].toNumber(), "war ID")
            (0, chai_1.expect)(defenderStruct[4].toNumber()).to.equal(0);
        });
        it("tests that battle odds work correctly", async function () {
            const battleOdds = await groundbattlecontract.battleOdds(0, 0);
            // console.log(battleOdds[0].toNumber(), "attacker victory odds", battleOdds[1].toNumber(), "defender victory odds")
            (0, chai_1.expect)(battleOdds[0]).to.equal(41);
            (0, chai_1.expect)(battleOdds[1]).to.equal(58);
        });
        it("tests that battle happens", async function () {
            ...
            for (const log of event1Logs) {
                ...
                if (requestIdReturn == 1) {
                    ...
                    const results = await groundbattlecontract.returnBattleResults(0);
                    console.log(results);
                    var attackerId = results[0].toNumber();
                    ...
                    var attackerDeployedForces = await warcontract.getDeployedGroundForces(0, 0);
        ...
        it("tests that casualties increase from ground battle", async function () {
            ...
            // console.log(casualties[0].toNumber(), "casualties");
            // console.log(casualties[1].toNumber(), "casualties");
            (0, chai_1.expect)(attackerSoldierLosses).to.equal(270);
            (0, chai_1.expect)(attackerTankLosses).to.equal(7);
            (0, chai_1.expect)(attackerSoldiers).to.equal(1750);
            (0, chai_1.expect)(attackerTanks).to.equal(33);
            (0, chai_1.expect)(casualties[0]).to.equal(270); //soldier casualties
            (0, chai_1.expect)(casualties[1]).to.equal(7); //tank casualties
        });
        it("tests that infrastructure damage occurs from ground battle", async function () {
            await infrastructuremarketplace.connect(signer1).buyInfrastructure(0, 5000);
            await forcescontract.connect(signer1).buySoldiers(10000, 0);
            await forcescontract.connect(signer1).buyTanks(500, 0);
            await keepercontract.incrementGameDay();
            await forcescontract.connect(signer1).deployForces(5000, 500, 0, 0);
            ...
            (0, chai_1.expect)(defenderLand).to.equal(20);
            (0, chai_1.expect)(defenderInfrastructure).to.equal(5020);
            await groundbattlecontract.connect(signer1).groundAttack(0, 0, 1, 1);
            const eventFilter1 = vrfCoordinatorV2Mock.filters.RandomWordsRequested();
            const event1Logs = await vrfCoordinatorV2Mock.queryFilter(eventFilter1);
            for (const log of event1Logs) {
            ...
            (0, chai_1.expect)(defenderLand).to.equal(18);
            (0, chai_1.expect)(defenderInfrastructure).to.equal(5018);
        });
    });
});
