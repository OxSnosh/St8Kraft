...
import { LinkTokenInterface } from '../typechain-types/@chainlink/contracts/src/v0.7/interfaces/LinkTokenInterface';
import { Operator } from '../typechain-types/contracts/tests/Operator';
...
describe("Air Battle", async function () {
    ...
    beforeEach(async function () {
        ...
        linkToken = await LinkToken.deploy() as LinkTokenInterface;
        ...
        operator = await Operator.deploy(linkToken.address, signer0.address) as Operator;
        await operator.deployed();

        console.log("setting authorized senders");
        await operator.setAuthorizedSenders([airbattlecontract.address])

        console.log("funding node")
        const oracleNodeAddress = "0x8063b78A31De41c7d232dA59E02D441430c6C505"
        // const signer0PrivateKey : any = "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"
        // const provider = ethers.getDefaultProvider("localhost")
        // const wallet = new ethers.Wallet(signer0PrivateKey, provider)
        // const amountOfEtherToSend = ethers.utils.parseEther("10.0")
        // let tx = {
        //     to: oracleNodeAddress,
        //     value: amountOfEtherToSend
        // }
        // console.log("here?")
        // const transaction : any = await wallet.sendTransaction(tx).then((txObj) => {
        //     console.log("funded node hash:", txObj.hash)
        // })
        
        const transaction = await signer0.sendTransaction({
            to: oracleNodeAddress,
            value: ethers.utils.parseEther("10.0")
        })
        await transaction.wait(1)
        var balanceOfNode = await ethers.provider.getBalance(oracleNodeAddress);
        console.log("balance of node:", balanceOfNode.toString())

                
        console.log("funding client contract");
        await linkToken.transfer(airbattlecontract.address, BigInt(10000000000000000000000000))
        const balance = await linkToken.balanceOf(airbattlecontract.address)
        console.log(balance.toString(), "link balance of contract")
    });

    describe("Air Battle", function () {
        it("tests array battle", async function () {
